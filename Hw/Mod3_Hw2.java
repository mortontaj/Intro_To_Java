...
//	static String palindrome(String word_phrase) {
//
...
/*		String phrase_word = "";
		...
		return word_phrase.concat(statement2);
*/
...
//	"""Return the `start to end` Fibonacci numbers for positive `start to end`."""
	...
	String word_phrase = input.next();
	//palindrome(word);
	
	if (!word_phrase.isBlank())
		{
			String phrase_word = "";
			for (int i = word_phrase.length() - 1; i >= 0; i--)
				{
			...
			System.out.println(phrase_word);
			String statement2 = (word_phrase.equals(phrase_word) ? " is a palindrome!" : " is not an palindrome.");
			System.out.println(word_phrase.concat(statement2));
		}
	} //closing class files
}
